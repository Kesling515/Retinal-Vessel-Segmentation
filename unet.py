...
# 定义网络模型：Unet
def get_unet(minimum_kernel=32, do=0, size=64, activation=ReLU):
...
def train(DATASET="DRIVE", crop_size=64, need_au=True, ACTIVATION='ReLU', dropout=0.2, batch_size=20,
    ...
    print('-' * 35)
    model = get_unet(minimum_kernel=minimum_kernel, do=dropout, size=crop_size, activation=activation)
